...
async def node_judge(state: GraphState) -> GraphState:
    from agent.judge import pipeline as judge_pipeline

    try:
        results = await judge_pipeline.evaluate_all(
            question=state["channel_message"].text,
        ...
        if failed:
            ...
            logger.warning("judge flagged interaction %s: %s", state["channel_message"].session_id, names)
    except Exception:
        logger.warning("judge falhou, continuando", exc_info=True)
