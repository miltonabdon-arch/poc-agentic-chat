...
| Observability Tracer | SPEC-007 (Observabilidade — Langfuse + OpenTelemetry + eventos IC/NOC/GRL) | Langfuse auto-hospedado (docker-compose `:3000`) + log local + broadcaster SSE. Eventos IC/NOC/GRL via `AgentObserver` com `LangfuseAnalyticsPublisher`; extensões locais FLOW/LLM/RAG/MOCK/JUDGE/GRAPH/ORCH/STATE via broadcaster. Sem Langfuse gerenciado em nuvem nem OTel collector externo. |
...
| Empacotamento | `docker-compose` com 4 serviços: `app` (gateway FastAPI), `mock-services` (agentes mock), `langfuse` (dashboard), `langfuse-db` (Postgres 15) — sem Kubernetes/OKE | Escopo de 2 semanas não comporta infraestrutura de orquestração de containers; Langfuse local elimina custo de nuvem para observabilidade |
